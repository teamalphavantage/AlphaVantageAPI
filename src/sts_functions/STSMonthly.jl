function process_raw_json_api_data_sts_(api_call_raw_data::String, time::String)::(Union{T, Nothing} where T<:Any)
...
function process_raw_json_api_data_sts_monthly(api_call_raw_data::String)::(Union{T, Nothing} where T<:Any)
    check_result = _check_json_api_return_data(api_call_raw_data)
    if check_result != nothing
        return check_result
    end
    process_raw_json_api_data_sts_(check_result, "Monthly Time Series")::(Union{T, Nothing} where T<:Any)
end
function execute_sts_monthly_api_call(user_model::AVKUserModel, stock_symbol::String; data_type::Symbol = :json, outputsize::Symbol = :compact, logger::Union{Nothing,AbstractLogger} = nothing)::(Union{T, Nothing} where T<:Any)
